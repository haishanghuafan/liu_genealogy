...
@router.get("/single-sheet/excel")
async def export_single_sheet_excel(
    request: Request,
    db: AsyncSession = Depends(get_tenant_db),
    user = Depends(get_current_user_required),
):
    """Export full genealogy data to single sheet Excel (matching import template)"""
    tenant = require_tenant(request)
    ...
    excel_file = await export_service.export_single_sheet_excel(db, tenant.slug)
